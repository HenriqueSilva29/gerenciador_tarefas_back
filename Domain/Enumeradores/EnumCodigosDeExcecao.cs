namespace Domain.Enumeradores
{
    public static class EnumCodigosDeExcecao
    {
        public const string LembreteJaEnviado = "LEMBRETE_JA_ENVIADO";
        public const string PossuiSubtarefaNaoFinalizada = "POSSUI_SUBTAREFA_NAO_FINALIZADA";
        public const string PossuiSubtarefaComPrioridadeMaiorQueTarefaPai = "POSSUI_SUBTAREFA_COM_PRIORIDADE_MAIOR_QUE_TAREFA_PAI";
        public const string UsuarioJaCadastrado = "USUARIO_JA_CADASTRADO";
        public const string RegistroNaoEncontrado = "REGISTRO_NAO_ENCONTRADO";
        public const string CredenciaisInvalidas = "CREDENCIAIS_INVALIDAS";
        public const string UsuarioNaoAutenticado = "USUARIO_NAO_AUTENTICADO";
        public const string RegistroSemUsuarioVinculado = "REGISTRO_SEM_USUARIO_VINCULADO";
    }
}

